...
lda = LinearDiscriminantAnalysis().fit(standardisedX)


def lda_summary(lda, standardised_data, out=True):
    names = ["PC"+str(i) for i in range(1, len(lda.explained_variance_ratio_)+1)]
...
print(np.sum(lda.components_[0]**2))
...
print(calc_pc(standardisedX, lda.components_[0]))
print(lda.transform(standardisedX)[:, 0])

print(lda.components_[1])
print(np.sum(lda.components_[1]**2))
